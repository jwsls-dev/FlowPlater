var FlowPlater=function(){"use strict";function getInstance(e){return _state.instances[e]}function getInstances(){return _state.instances}function instanceMethods(c){return{instanceName:c,animate:_state.defaults.animation,_updateDOM:async function(){const r=_state.instances[c];var e;if(r)try{let t;if("self"===r.templateId||null===r.templateId){var n=Array.from(r.elements)[0];if(!n)return void Debug.error("No template element found for self template",r.instanceName);t=n.innerHTML}else{if(!r.template)return void Debug.error("No template found for instance",r.instanceName);var a,{valid:o,isHtml:i}="string"==typeof(e=r.data)&&e.trim().startsWith("<!DOCTYPE")?(Debug.debug("Data is HTML, skipping validation",c),{valid:!0,isHtml:!0}):"object"==typeof e&&null!==e||Array.isArray(e)||"number"==typeof e||"boolean"==typeof e?{valid:!0,isHtml:!1}:(Debug.error("Invalid data type: "+typeof e),{valid:!1,isHtml:!1});if(!o)return void Debug.error("Invalid data type for instance",r.instanceName);i?(Debug.debug("Data is HTML, using as is",r.instanceName),t=r.data):(Debug.debug("Before transformation - instance data:",{instanceName:r.instanceName,rawData:r.data,getData:r.getData()}),a=PluginManager.applyTransformations(r,r.getData(),"transformDataBeforeRender","json"),Debug.debug("After transformation - transformed data:",{instanceName:r.instanceName,transformedData:a,isNull:null===a,isUndefined:void 0===a,type:typeof a}),t=r.template(a),Debug.debug("Rendered template with data:",{template:r.templateId,data:a,rendered:t}))}var s,l=Array.from(r.elements).filter(e=>document.body.contains(e));if(0!==l.length)return s=l.map(e=>domBatcher.write(()=>updateDOM(e,t,r.animate,r),`update-${r.instanceName}-`+Date.now())),await Promise.all(s);Debug.error("No active elements found for instance",r.instanceName)}catch(e){Debug.error("Error updating DOM for instance",r.instanceName,e)}else Debug.error("Instance not found: "+c)},setData:function(e){var t=_state.instances[c];if(t)if("string"!=typeof e&&"number"!=typeof e&&!Array.isArray(e)||(Debug.warn("[setData] Received raw value or unnamed root object, automatically wrapping in 'data' property"),e={data:e}),"object"!=typeof e||null===e||Array.isArray(e))Debug.error("Invalid newData type provided to setData: "+typeof e);else{var r=t.data,n=(Debug.debug(`[setData] Replacing data for ${c}. Current keys: ${Object.keys(r).join(", ")}, New keys: `+Object.keys(e).join(", ")),[]);for(const a in r)Object.hasOwnProperty.call(r,a)&&!Object.hasOwnProperty.call(e,a)&&(n.push(a),delete r[a]);0<n.length&&Debug.debug(`[setData] Deleted stale keys for ${c}: `+n.join(", ")),deepMerge(r,e),Debug.debug(`[setData] Data replacement complete for ${c}.`)}else Debug.error("Instance not found: "+c);return this},remove:function(){var e=_state.instances[c];if(!e)throw new Error("Instance not found: "+c);EventSystem.publish("beforeRemove",{instanceName:c,elements:e.elements});try{return ConfigManager.getConfig().storage?.enabled&&localStorage.removeItem("fp_"+c),e.elements.forEach(function(e){try{e.innerHTML=""}catch(e){Debug.error("Error removing instance: "+e.message)}}),e.elements=[],delete _state.instances[c],delete _state.templateCache[e.templateId],EventSystem.publish("afterRemove",{instanceName:c,elements:[]}),Debug.info("Removed instance: "+c),!0}catch(e){throw e}},refresh:async function(i={remote:!0,recompile:!1,ignoreLocalData:!1}){const s=_state.instances[c];if(!s)return Debug.error("Instance not found: "+c),Promise.reject(new Error("Instance not found: "+c));var e=PluginManager.applyTransformations(s,s.data,"transformDataBeforeRender","json"),t=s.template(e);(i.recompile||!t&&s.data)&&(s.template=compileTemplate(s.templateId,!0)),Debug.debug("Refresh - Template check:",{templateId:s.templateId,templateElement:document.querySelector(s.templateId),compiledTemplate:s.template(e)});let l=!1;s.localVarName&&!i.ignoreLocalData&&(t=extractLocalData(s.localVarName))&&(deepMerge(s.data,t),l=!0,Debug.debug(`Refreshed data from local variable "${s.localVarName}"`));const u=[];return s.elements.forEach(async function(t){try{if(i.remote&&!l){var e,r,n,a=["get","post","put","patch","delete"];a.some(e=>t.getAttribute("hx-"+e))&&(e=a.find(e=>t.getAttribute("hx-"+e)),r=t.getAttribute("hx-"+e),n=fetch(r,{method:e?.toUpperCase()}).then(e=>{if(e.ok)return e.json();throw new Error("HTTP error! status: "+e.status)}).then(e=>(deepMerge(s.data,e),ConfigManager.getConfig().storage?.enabled&&saveToLocalStorage(c,s.data,"instance"),e)),u.push(n))}else{const o=PluginManager.applyTransformations(s,s.data,"transformDataBeforeRender","json");Debug.debug("Instance.refresh - Transformed data:",o),u.push(domBatcher.write(()=>updateDOM(t,s.template(o),s.animate,s),`refresh-${c}-`+Date.now()))}}catch(e){t.innerHTML=`<div class="fp-error">Error refreshing template: ${e.message}</div>`,Debug.error("Failed to refresh template: "+e.message),u.push(Promise.reject(e))}}),Promise.all(u)},getData:function(){var e=_state.instances[c].data;return JSON.parse(JSON.stringify(e))},getElements:function(){return _state.instances[c].elements},getTemplateElement:function(){return _state.instances[c].templateElement},get:function(e){return e?function(e){var r=_state.instances[c];if(r){var n=e.split(/[\.\[\]'"]/);let t=r.data;for(let e=0;e<n.length;e++){var a=n[e];if(""!==a){if(void 0===t||null===t||!t.hasOwnProperty(a))return;t=t[a]}}return t}Debug.error("Instance not found: "+c)}.call(this,e):this.getData?.()},refreshTemplate:function(e,t=!1){return Performance.start("refreshTemplate:"+e),!!compileTemplate(e,t)||(Debug.error("Failed to compile template: "+e),Performance.end("refreshTemplate:"+e),!1)}}}function memoize(r){const n=new Map;var e=(...e)=>{var t=JSON.stringify(e);return n.has(t)?n.get(t):(e=r(...e),n.set(t,e),e)};return e.original=r,e.cache=n,e}function setCustomTags(e){currentCustomTags=e}function replaceCustomTags(e){let t=!1;for(const a of currentCustomTags){var r=e.getElementsByTagName(a.tag);if(0<r.length){t=!0;for(const o of Array.from(r)){var n=document.createElement(a.replaceWith);n.innerHTML=o.innerHTML;for(const i of o.attributes)n.setAttribute(i.name,i.value);o.parentNode?.replaceChild(n,o)}}}return t&&Debug.info("replaced custom tags",e),e}function compileTemplate(e,t=!1){if(!t)return memoizedCompile(e);delete _state.templateCache[e];t=memoizedCompile.original(e);return memoizedCompile.cache.set(JSON.stringify([e]),t),t}function render({template:a,data:o,target:e,returnHtml:i=!1,instanceName:s,animate:l=_state.defaults.animation,recompile:u=!1,skipLocalStorageLoad:c=!1,skipRender:p=!1,isStoredDataRender:h=!1}){Performance.start("render:"+(s||"anonymous")),_state._initTracking||(_state._initTracking={});let t;if(s?t=s:e instanceof Element&&AttributeMatcher._hasAttribute(e,"instance")?t=AttributeMatcher._getRawAttribute(e,"instance"):e instanceof Element&&e.id?t=e.id:"string"==typeof e&&e.startsWith("#")&&(t=e.substring(1)),t&&_state._initTracking[t]&&!h){h=Date.now()-_state._initTracking[t];if(h<100)return Debug.warn(`[Template] Skipping redundant initialization for ${t}, last init was ${h}ms ago`),_state.instances[t]||null}t&&(_state._initTracking[t]=Date.now()),EventSystem.publish("beforeRender",{instanceName:s,template:a,data:o,target:e,returnHtml:i,recompile:u}),a&&"self"!==a||(h="string"==typeof e?document.querySelector(e):e)&&(a="#"+h.id);let d=[];if(0===(d=e instanceof Element&&(h=AttributeMatcher._getRawAttribute(e,"target"))?"this"===h?[e]:Array.from(document.querySelectorAll(h)):d).length&&(e instanceof NodeList?d=Array.from(e):"string"==typeof e?d=Array.from(document.querySelectorAll(e)):e instanceof Element&&(d=[e])),0===d.length)Debug.error("No target elements found");else{s=s||(AttributeMatcher._hasAttribute(d[0],"instance")?AttributeMatcher._getRawAttribute(d[0],"instance")||void 0:d[0].id||_state.length.toString());var g=compileTemplate(a,u);if(_state.length++,g){if(0!==d.length){let t=o||{},r=null,n=null;const v=AttributeMatcher._getRawAttribute(d[0],"local");let e=null;if(v){if(!(e=extractLocalData(v)))try{var f=document.querySelector(v);f&&AttributeMatcher.findMatchingElements("data",null,!1,f)&&(m=PluginManager.getPlugin("data-extractor"))&&(e=m.instanceMethods?.extractData?.(f))}catch(e){Debug.warn("[Template] Invalid selector for fp-local: "+v)}e&&(t={...t,...e}),AttributeMatcher._hasAttribute(d[0],"local-observe")&&AttributeMatcher.findMatchingElements("data")?.forEach(e=>{PluginManager.getPlugin("data-extractor")?.instanceMethods?.observeDataElement?.(e,v,s?_state.instances[s]:void 0)})}if(s&&_state.instances[s]&&_state.instances[s].data)n=_state.instances[s].data;else{if(!c&&ConfigManager.getConfig().storage?.enabled&&(r=loadFromLocalStorage(s||"","instance"))){if(!0===r.isHtml||"string"==typeof r&&"function"==typeof r.trim&&(r.trim().startsWith("<!DOCTYPE html")||r.trim().startsWith("<html"))){h=AttributeMatcher._getRawAttribute(d[0],"swap","innerHTML");const E={swapStyle:h||"innerHTML",swapDelay:0,settleDelay:0,transition:h?.includes("transition:true")||!1};return i?r:(d.forEach(e=>{htmx.swap(e,r,E)}),s?_state.instances[s]:void 0)}u=r?.data||r;t={...u,...t},Debug.debug(`[Template] Merged persisted data for ${s}:`,t)}o=d.find(e=>AttributeMatcher._hasAttribute(e,"template"));if(!o)return Debug.error("No template element found in target elements"),null;const y=InstanceManager.getOrCreateInstance(o,t);if(!y)return Debug.error("Failed to get or create instance: "+s),null;v&&(y.localVarName=v),y._updateTimer||(y._updateTimer=null),y._stateBeforeDebounce||(y._stateBeforeDebounce=null);var m=AttributeMatcher._getRawAttribute(o,"group");if(m){let e=t;!c&&ConfigManager.getConfig().storage?.enabled&&(f=loadFromLocalStorage(m,"group"))&&(e=deepMerge(f,t),Debug.debug("Loaded persisted data for group "+m,e));h=GroupManager.getOrCreateGroup(m,e);n=h.data,GroupManager.addInstanceToGroup(y,m),Debug.info(`Instance ${s} is using group ${m} data`)}else n=createDeepProxy(t,()=>{y&&!y._isEvaluating&&(y._updateTimer&&clearTimeout(y._updateTimer),y._updateTimer=setTimeout(()=>{try{y._isEvaluating=!0;var e,t=PluginManager.applyTransformations(y,y.getData(),"transformDataBeforeRender","json"),r=y.template(t);y._lastRenderedOutput!==r?(Debug.info(`[Debounced Update] Output changed for ${s}. Firing updateData hook.`),PluginManager.executeHook("updateData",y,{newData:n,source:"proxy"}),EventSystem.publish("updateData",{instanceName:s,newData:n,source:"proxy"}),Debug.debug("[Debounced Update] Triggering _updateDOM for "+s),y._updateDOM(),y._lastRenderedOutput=r,ConfigManager.getConfig().storage?.enabled&&(e=y.instanceName.replace("#",""),Debug.debug("[Debounced Update] Saving data for "+e),saveToLocalStorage(e,n,"instance"))):Debug.debug(`[Debounced Update] No output change for ${s}. Skipping update.`)}finally{y._isEvaluating=!1,y._updateTimer=null}},0))});y.template=g,y.templateId=AttributeMatcher._getRawAttribute(o,"template")||a,y.data=n,Debug.debug(`[Initial Render] ${p?"Skipping":"Triggering"} _updateDOM for `+s),p||y._updateDOM(),!ConfigManager.getConfig().storage?.enabled||r||y.groupName||(u=s?.replace("#","")||"",Debug.debug("[Initial Save] Saving initial data for "+u),saveToLocalStorage(u,t,"instance"))}c=s?_state.instances[s]:void 0;if(c&&Debug.info("Final instance data: ",c.data),n){var b,f=d.find(e=>AttributeMatcher._hasAttribute(e,"template"));if(!f)return Debug.error("No template element found in target elements"),null;const S=InstanceManager.getOrCreateInstance(f,t);if(!S)return Debug.error("Failed to get or create instance: "+s),null;if(S.template=g,S.templateId=AttributeMatcher._getRawAttribute(f,"template")||a,S.data=n,p)Debug.debug(`[Render Template] Skipping render for ${s} as requested`);else{Debug.debug("[Render Template] Executing render for "+s);try{if(i)return b=PluginManager.applyTransformations(S,S.getData(),"transformDataBeforeRender","json"),g(b);Array.from(S.elements).forEach(e=>{var t=PluginManager.applyTransformations(S,S.getData(),"transformDataBeforeRender","json");updateDOM(e,g(t),l,S)})}catch(e){throw e instanceof TemplateError||Debug.error("Failed to render template: "+e.message),e}}}else p||Debug.debug(`[Template] Skipping render for ${s} because no data is available yet.`);return c||null}Debug.error("Target not found: "+e)}else Debug.error("Template not found: "+a)}}function normalizeStorageConfig(e,t){return"boolean"==typeof e?{enabled:e,ttl:t.ttl}:"number"==typeof e?-1===e?{enabled:!0,ttl:-1}:{enabled:0<e,ttl:0<e?e:t.ttl}:"object"==typeof e&&null!==e?{enabled:!!e.enabled,ttl:("number"==typeof e.ttl?e:t).ttl}:t}function applyConfigInternal(e){"storage"in e&&(e.storage=normalizeStorageConfig(e.storage,defaultConfig.storage)),_state.config=deepMerge(JSON.parse(JSON.stringify(_state.config)),e),Debug.level=_state.config.debug.level,"undefined"!=typeof htmx&&(htmx.config.timeout=_state.config.htmx.timeout,htmx.config.defaultSwapStyle=_state.config.htmx.swapStyle,htmx.config.selfRequestsOnly=_state.config.htmx.selfRequestsOnly,htmx.config.ignoreTitle=_state.config.htmx.ignoreTitle),e.customTags&&setCustomTags(e.customTags)}function replacer(e,t){return t instanceof Map?{_dataType:"Map",value:Array.from(t.entries())}:t}function reviver(e,t){return"object"==typeof t&&null!==t&&"Map"===t._dataType?new Map(t.value):t}function saveToLocalStorage(t,r,n=""){var a=ConfigManager.getConfig().storage;if(a?.enabled)try{var o=n?`fp_${n}_`+t:"fp_"+t;let e;try{e=JSON.stringify(r,replacer)}catch(e){return Debug.error(`[LocalStorage] Failed to serialize data for key ${o}: `+e.message)}var i={data:e,expiry:-1===a.ttl?null:Date.now()+1e3*a.ttl};Debug.debug("[LocalStorage] Saving to localStorage:",{key:o,item:i}),localStorage.setItem(o,JSON.stringify(i))}catch(e){Debug.error(`[LocalStorage] Failed to save to localStorage for key ${t}: `+e.message)}else Debug.debug("[LocalStorage] Storage is disabled, skipping save for key: "+t)}function loadFromLocalStorage(t,e=""){try{var r,n,a=e?`fp_${e}_`+t:"fp_"+t,o=localStorage.getItem(a);return o?(r=JSON.parse(o)).expiry&&r.expiry<Date.now()?(Debug.debug("[LocalStorage] Stored item has expired for key: "+a),localStorage.removeItem(a),null):(n=JSON.parse(r.data,reviver),Debug.debug("[LocalStorage] Loaded from localStorage:",{key:a,data:n}),n):(Debug.debug("[LocalStorage] No stored item found for key: "+a),null)}catch(e){return Debug.error(`[LocalStorage] Failed to load or parse from localStorage for key ${t}: `+e.message),null}}function createDeepProxy(e,n){return"object"!=typeof e||null===e?e:new Proxy(e,{get(e,t){e=e[t];return e&&"object"==typeof e?createDeepProxy(e,n):e},set(e,t,r){return e[t]=r,Promise.resolve().then(()=>n(e)),!0},deleteProperty(e,t){return delete e[t],Promise.resolve().then(()=>n(e)),!0}})}function deepMerge(e,t){if(t){if(!e)return t;for(const r in t)Object.prototype.hasOwnProperty.call(t,r)&&(Array.isArray(e[r])&&Array.isArray(t[r])||!(r in e)||"object"!=typeof e[r]||"object"!=typeof t[r]||null===e[r]||null===t[r]?e[r]=t[r]:deepMerge(e[r],t[r]))}return e}function collectDebugInfo(e,t,r={}){return{formId:e.id,type:t,persistenceEnabled:isPersistenceEnabledForElement(e),restoredElements:r.restoredElements||[],customVisualUpdates:r.customVisualUpdates||[],skippedElements:r.skippedElements||[],storageType:r.storageType}}function handleFormStorage(e,t={},r="save"){var n=shouldUseLocalStorage(e),a="fp_form_"+e.id;if("save"===r)n?saveToLocalStorage(e.id,t,"form"):sessionStorage.setItem(a,JSON.stringify(t));else{if("load"===r)return n?loadFromLocalStorage(e.id,"form"):JSON.parse(sessionStorage.getItem(a)||"{}");"clear"===r&&(n&&localStorage.removeItem(a),sessionStorage.removeItem(a))}}function processFormElements(e,t){Array.from(e.elements).forEach(e=>{(e instanceof HTMLInputElement||e instanceof HTMLSelectElement||e instanceof HTMLTextAreaElement)&&e.name&&"file"!==e.type&&isPersistenceEnabledForElement(e)&&t(e)})}function manageEventListener(t,e,r,n="add"){("change"===e?["change","checkbox"!==t.type&&"radio"!==t.type?"input":null]:[e]).filter(Boolean).forEach(e=>{t["add"===n?"addEventListener":"removeEventListener"](e,r)})}function restoreElementValue(e,t){e instanceof HTMLInputElement&&("checkbox"===e.type||"radio"===e.type)?(e.checked="true"===t,updateCustomVisualState(e)):e instanceof HTMLSelectElement&&e.multiple?Array.from(e.options).forEach(e=>{e.selected=t.includes(e.value)}):e.value=t}function updateCustomVisualState(e){var t=e.closest("checkbox"===e.type?".w-checkbox":".w-radio");t&&(t=t.querySelector(`.w-${e.type}-input`))&&t.classList.toggle("w--redirected-checked",e.checked)}function getPersistenceSettings(e){let t=!1,r=!1;var n;return r=AttributeMatcher._hasAttribute(e,"persist")?(t="false"!==AttributeMatcher._getRawAttribute(e,"persist"),"true"===AttributeMatcher._getRawAttribute(e,"persist")):(n=e.closest("form"))&&AttributeMatcher._hasAttribute(n,"persist")?(t="false"!==AttributeMatcher._getRawAttribute(n,"persist"),"true"===AttributeMatcher._getRawAttribute(n,"persist")):ConfigManager.getConfig().storage?.enabled&&!ConfigManager.getConfig().persistForm?t=!1:(t=ConfigManager.getConfig().persistForm,ConfigManager.getConfig().storage?.enabled&&ConfigManager.getConfig().persistForm),e instanceof HTMLFormElement&&Array.from(e.elements).some(e=>"true"===AttributeMatcher._getRawAttribute(e,"persist"))&&(r=ConfigManager.getConfig().storage?.enabled),{shouldPersist:t,useLocalStorage:r&&ConfigManager.getConfig().storage?.enabled}}function isPersistenceEnabledForElement(e){return getPersistenceSettings(e).shouldPersist}function shouldUseLocalStorage(e){return getPersistenceSettings(e).useLocalStorage}function captureFormStates(e){try{var t=e.getElementsByTagName("form");const r={};return Array.from(t).forEach(e=>{if(e.id){const t={};Array.from(e.elements).forEach(e=>{e.name&&"file"!==e.type&&isPersistenceEnabledForElement(e)&&("checkbox"===e.type||"radio"===e.type?t[e.name]=e.checked:e instanceof HTMLSelectElement?t[e.name]=e.multiple?Array.from(e.selectedOptions).map(e=>e.value):e.value:t[e.name]=e.value)}),0<Object.keys(t).length&&(EventSystem.publish("formState:beforeCapture",{formId:e.id,formElement:e,state:t}),r[e.id]=t,shouldUseLocalStorage(e)?saveToLocalStorage(e.id,t,"form"):sessionStorage.setItem("fp_form_"+e.id,JSON.stringify(t)))}}),r}catch(e){return Debug.error("Error capturing form states: "+e.message),{}}}function restoreSingleFormState(e,t){if(e.id){const r=handleFormStorage(e,{},"load");if(r){const n=collectDebugInfo(e,"restore",{restoredElements:[],customVisualUpdates:[],skippedElements:[],storageType:shouldUseLocalStorage(e)?"localStorage":"sessionStorage"});processFormElements(e,e=>{e.name in r&&(n.restoredElements.push({name:e.name,value:r[e.name]}),restoreElementValue(e,r[e.name]))}),Debug.debug("Form state restoration summary for "+e.id,{storageType:n.storageType,source:t||"unknown",restoredElements:n.restoredElements.map(e=>({name:e.name,value:e.value})),updatedCustomVisualStates:n.customVisualUpdates,skippedElements:n.skippedElements}),EventSystem.publish("formState:afterRestore",{formId:e.id,formElement:e,state:r,source:t||"unknown"})}else Debug.debug("No stored state found for form: "+e.id)}}function clearFormState(e){FormStateManager.clearFormState(e)}function setupFormChangeListeners(r){try{if(r.id){const n=collectDebugInfo(r,"setup",{formElements:r.elements.length,checkboxWrappers:r.querySelectorAll(".w-checkbox").length,listenersAdded:[],skippedElements:[]});r._fpChangeListeners||(r._fpChangeListeners=[]),processFormElements(r,t=>{var e;r._fpChangeListeners.some(({element:e})=>e===t)||(manageEventListener(t,"change",e=e=>handleFormElementChange(e)),r._fpChangeListeners.push({element:t,handler:e}),n.listenersAdded?.push(t.name))}),Debug.debug("Form setup summary for "+r.id,{totalFormElements:n.formElements,checkboxWrappers:n.checkboxWrappers,formPersistence:n.persistenceEnabled?"enabled":"disabled",listenersAdded:n.listenersAdded?.join(", "),skippedElements:n.skippedElements.join(", ")})}else Debug.debug("Skipping form without ID")}catch(e){Debug.error("Error setting up form change listeners: "+e.message)}}function cleanupFormChangeListeners(e){try{e._fpChangeListeners&&(e._fpChangeListeners.forEach(({element:e,handler:t})=>{e.removeEventListener("change",t),e.removeEventListener("input",t)}),e._fpChangeListeners=[])}catch(e){Debug.error("Error cleaning up form change listeners: "+e.message)}}function handleFormElementChange(e){try{var t=e.target;const a=t.closest("form");if(a&&a.id){const o=collectDebugInfo(a,"change",{changedValues:{},skippedElements:[]}),i={};if(processFormElements(a,e=>{var t;isTemplateValue((t=e).name)||isTemplateValue(t.value)||AttributeMatcher._hasAttribute(t,"bind")?o.skippedElements.push({name:e.name,reason:"Template binding detected",value:e.value}):(t="checkbox"===e.type||"radio"===e.type?e.checked:e instanceof HTMLSelectElement&&e.multiple?Array.from(e.selectedOptions).map(e=>e.value):e.value,i[e.name]=t,o.changedValues||(o.changedValues={}),o.changedValues[e.name]=t)}),0<Object.keys(i).length){handleFormStorage(a,i,"save"),Debug.debug("Form state update for "+a.id+":",{"Changed element":t.name,"Storage type":shouldUseLocalStorage(a)?"localStorage":"sessionStorage","Updated values":o.changedValues,"Skipped elements":o.skippedElements});let e=null;for(var[r,n]of Object.entries(_state.instances))if(Array.from(n.elements).some(e=>e.contains(a)||a.contains(e)||e===a)){e=n;break}PluginManager.executeHook("updateForm",e,{element:a,id:a.id,data:i,changedElement:t}),EventSystem.publish("formState:changed",{formId:a.id,formElement:a,state:i,changedElement:t})}function isTemplateValue(e){return"string"==typeof e&&(e.includes("{{")||e.includes("}}")||e.includes("[[")||e.includes("]]")||e.includes("this."))}}else Debug.debug("Skipping change handler - no form or form ID")}catch(e){Debug.error("Error handling form element change: "+e.message)}}function getAllRelevantForms(e){const t=new Set;e instanceof HTMLFormElement&&t.add(e);var r=e.closest("form"),r=(r&&t.add(r),e.getElementsByTagName("form"));return Array.from(r).forEach(e=>t.add(e)),t}function setupFormSubmitHandlers(e,t){try{Debug.debug("Setting up form submit handlers for element:",e);var r=getAllRelevantForms(e);Debug.debug(`Found ${r.size} forms`),r.forEach(e=>{setupSingleFormHandlers(e,t||"setupFormSubmitHandlers")})}catch(e){Debug.error("Error setting up form submit handlers: "+e.message)}}function setupSingleFormHandlers(e,t){e.id?(Debug.debug("Setting up handlers for form: "+e.id),e.removeEventListener("submit",handleFormSubmit),e.addEventListener("submit",handleFormSubmit),setupFormChangeListeners(e),e._fpHandlersSetup=!0,shouldRestoreForm(e)?(Debug.debug("Restoring state for form: "+e.id),restoreSingleFormState(e,t||"setupSingleFormHandlers")):Debug.debug("Skipping form restoration - no persistent elements: "+e.id)):Debug.debug("Skipping form without ID")}function handleFormSubmit(e){try{var t=e.target.closest("form");t&&t.id&&clearFormState(t.id)}catch(e){Debug.error("Error handling form submit: "+e.message)}}function setupDynamicFormObserver(e){try{var t=new MutationObserver(e=>{e.forEach(e=>{e.addedNodes.forEach(e=>{e instanceof HTMLFormElement&&setupFormSubmitHandlers(e,"setupDynamicFormObserver")})})});return t.observe(e,{childList:!0,subtree:!0}),t}catch(e){return Debug.error("Error setting up dynamic form observer: "+e.message),null}}function shouldRestoreForm(e){return FormStateManager.shouldRestoreForm(e)}function isCssSelector(e){return e.startsWith("#")||e.startsWith(".")||/[[]>+~:()]/.test(e)||/^[a-z]+[a-z-]+$/i.test(e)}function extractLocalData(t,e=!0){if(!t)return null;try{if(isCssSelector(t)){var r=document.querySelector(t);if(r){var n=PluginManager.getPlugin("data-extractor");if(!n||"function"!=typeof n.globalMethods?.processHtml)return Debug.warn("DataExtractor plugin not available for element extraction"),null;try{var a=n.globalMethods?.processHtml?.(r.outerHTML);return Debug.debug(`Extracted data from element "${t}":`,a),a}catch(e){return Debug.error(`DataExtractor failed for "${t}": `+e.message),null}}}var o,i=window[t];return void 0!==i?e&&("object"!=typeof i||null===i||Array.isArray(i))?(o={[t]:i},Debug.info(`Wrapped primitive value "${t}":`,o),o):(Debug.info(`Extracted value "${t}":`,i),i):(Debug.warn(`Neither variable nor element found for "${t}"`),null)}catch(e){return Debug.info(`Error extracting local data "${t}": `+e.message),null}}async function performDomUpdate(t,e,r,n,a){if(r)await domBatcher.write(()=>{t.innerHTML=e,Debug.debug("performDomUpdate: Force full update completed for "+n)},`full-update-${n}-`+a);else{var r=document.createElement("div"),o=(r.innerHTML=e,0<r.children.length),r=r.textContent&&0<r.textContent.trim().length;if(o||r){Debug.debug(`performDomUpdate: Using Virtual DOM for element ${n} with ${t.childNodes.length} child nodes`),Debug.debug("performDomUpdate: Element innerHTML length: "+t.innerHTML.length),Debug.debug("performDomUpdate: Element innerHTML preview:",t.innerHTML.substring(0,200));const i=VirtualDOM.domToVNodes(t)||[],s=(Debug.debug(`performDomUpdate: Captured initial DOM state with ${i.length} nodes`),Debug.debug("performDomUpdate: Initial DOM nodes:",i.map((e,t)=>"string"==typeof e?t+`: "${e}"`:t+`: <${e.tag}${e.attrs.class?` class="${e.attrs.class}"`:""}>`)),VirtualDOM.parseHTML(e)||[]);Debug.debug(`performDomUpdate: Parsed new HTML into ${s.length} nodes`),Debug.debug("performDomUpdate: New DOM nodes:",s.map((e,t)=>"string"==typeof e?t+`: "${e}"`:t+`: <${e.tag}${e.attrs.class?` class="${e.attrs.class}"`:""}>`)),await domBatcher.batch([{type:"write",operation:()=>{var e=VirtualDOM.diff(i,s);Debug.debug(`performDomUpdate: Generated ${e.length} patches`),0<e.length?VirtualDOM.patch(t,e):Debug.debug("performDomUpdate: No patches to apply - content is identical")},id:`vdom-patch-${n}-`+a}])}else Debug.debug("performDomUpdate: Empty newHTML detected, clearing container for element "+n),await domBatcher.write(()=>{t.innerHTML=""},`clear-empty-${n}-`+a)}}async function updateDOM(r,n,a=!1,o=null){Performance.start("updateDOM");const i=AttributeMatcher._hasAttribute(r,"force-full-update");var s=ConfigManager.getConfig();const l=r.id||"unknown",u=Date.now();if(Debug.debug(`UpdateDOM: ===== STARTING UPDATE for ${l} at ${u} =====`),r.hasAttribute("fp-restoring")&&!i)Debug.debug(`UpdateDOM: Already restoring element ${l}, skipping update`);else{var e=parseInt(r.getAttribute("fp-update-counter")||"0")+1;r.setAttribute("fp-update-counter",e.toString()),r.setAttribute("fp-restoring","true"),Debug.debug(`UpdateDOM: This is update #${e} for element `+l);try{if(!(r&&r instanceof HTMLElement))throw new Error("Invalid target element");if("string"!=typeof n)throw new Error("newHTML must be a string");Debug.debug(`UpdateDOM called for element ${l} with newHTML length: `+n.length),0===n.length?Debug.warn("UpdateDOM: Empty newHTML provided for element "+l):Debug.debug("UpdateDOM: newHTML preview (first 200 chars): "+n.substring(0,200));const c=s.persistForm&&FormStateManager.shouldRestoreForm(r),p=c&&i;
// Virtual DOM already preserves form element state during normal updates
Debug.debug("Starting updateDOM with config:",s),Debug.debug(`Form persistence enabled: ${s.persistForm}, Should handle form state: ${c}, Needs manual preservation (force update): `+p);let t=null,e=null;p&&(Debug.debug("Capturing form states before update (manual preservation needed)"),t=await domBatcher.read(()=>captureFormStates(r),`capture-form-${l}-`+u),Debug.debug("Captured form states:",t)),c&&(Debug.debug("Setting up dynamic form observer"),e=setupDynamicFormObserver(r));const h=()=>new Promise(async e=>{EventSystem.publish("beforeDomUpdate",{element:r,newHTML:n,animate:a,formStates:t}),o&&PluginManager.executeHook("beforeDomUpdate",o,{element:r,newHTML:n,animate:a,formStates:t}),await performDomUpdate(r,n,i,l,u),c&&(p&&t&&(Debug.debug("Restoring form states after update (manual preservation)"),await domBatcher.write(()=>{FormStateManager.restoreFormStates(r,"updateDOM - form state restoration - restoreFormStates")},`restore-form-${l}-`+u)),Debug.debug("Setting up form submit handlers"),await domBatcher.write(()=>{setupFormSubmitHandlers(r,"updateDOM - form state restoration - setupFormSubmitHandlers")},`setup-form-handlers-${l}-`+u)),o&&PluginManager.executeHook("afterDomUpdate",o,{element:r,newHTML:n,animate:a,formStates:t}),EventSystem.publish("afterDomUpdate",{element:r,newHTML:n,animate:a,formStates:t}),e()});document.startViewTransition&&a?await document.startViewTransition(()=>h()).finished:await h(),e&&(Debug.debug("Disconnecting form observer"),e.disconnect())}catch(e){throw Debug.error("Error in updateDOM:",e),console.error("UpdateDOM error:",e),e}finally{r.removeAttribute("fp-restoring"),Debug.debug(`UpdateDOM: ===== COMPLETED UPDATE #${e} for ${l} =====`),Performance.end("updateDOM")}}}function setupAnimation(t){try{var e;return"true"===(AttributeMatcher._getRawAttribute(t,"animation")||_state.defaults.animation)&&(e=t.getAttribute("hx-swap"),t.setAttribute("hx-swap",e?e+" transition:true":"innerHTML transition:true")),t}catch(e){return Debug.error("Error in setupAnimation: "+e.message),t}}function compare(e,t,r){if(isNaN(e)||(e=Number(e)),isNaN(r)||(r=Number(r)),null==e||null==r)switch(t){case"==":return e==r;case"!=":return e!=r;case"&&":return Boolean(e)&&Boolean(r);case"||":return Boolean(e)||Boolean(r);default:return}if("string"==typeof e&&"string"==typeof r)switch(t){case"==":return 0===e.localeCompare(r);case"!=":return 0!==e.localeCompare(r);case"<":return e.localeCompare(r)<0;case">":return 0<e.localeCompare(r);case"<=":return e.localeCompare(r)<=0;case">=":return 0<=e.localeCompare(r);default:throw new TemplateError("Unsupported operator for strings: "+t)}switch(t){case"==":return e==r;case"!=":return e!=r;case"<":return e<r;case">":return r<e;case"<=":return e<=r;case">=":return r<=e;case"&&":return Boolean(e)&&Boolean(r);case"||":return Boolean(e)||Boolean(r);case"regex":return new RegExp(r).test(e);default:throw new TemplateError("Unsupported operator: "+t)}}function ifHelper(){var e=window.Handlebars;e.unregisterHelper("if"),e.registerHelper("if",function(e,t){function resolveValue(t,e,r){if(t.startsWith('"')&&t.endsWith('"')||t.startsWith("'")&&t.endsWith("'"))return t.slice(1,-1);if(!isNaN(Number(t)))return parseFloat(t);if("this"===t)return r;if(t.startsWith("this.")){const n=t.split(".").slice(1);let e=r;for(const o of n){if(!e||"object"!=typeof e)return;if(!Object.prototype.hasOwnProperty.call(e,o))return;e=e[o]}return e}const n=t.split(".");let a=e;for(const i of n){if(!a||"object"!=typeof a)return;if(!Object.prototype.hasOwnProperty.call(a,i))return;a=a[i]}return a}try{if(!e.match(/\s*(==|!=|<=|>=|<|>|\|\||&&)\s*/))return void 0!==resolveValue(e,t.data.root,this)?t.fn(this):t.inverse(this);var r=e.trim(),[n,a,o]=r.split(/\s*(==|!=|<=|>=|<|>|\|\||&&)\s*/);if(!n||a&&!o||!n&&a)throw new TemplateError("Invalid expression format: "+r);var i=resolveValue(n,t.data.root,this),s=!o||resolveValue(o,t.data.root,this),a=a||"==";return Debug.info("Evaluating expression:",{raw:r,leftValue:i,operator:a,rightValue:s}),"=="!==a||o?compare(i,a,s)?t.fn(this):t.inverse(this):void 0!==i?t.fn(this):t.inverse(this)}catch(e){throw e instanceof TemplateError||Debug.error("Error evaluating if condition:",e.stack),e}})}function sumHelper(){window.Handlebars.registerHelper("sum",function(...e){for(var t=0,r=0;r<e.length-1;r++){var n=e[r];Array.isArray(n)?n.forEach(function(e){t+=e}):t+=n}return t})}function sortFunction(r,n){return function(e,t){e=r?e[r]:e,t=r?t[r]:t;return null==e?n?-1:1:null==t?n?1:-1:"string"==typeof e&&"string"==typeof t?n?t.localeCompare(e):e.localeCompare(t):e<t?n?1:-1:t<e?n?-1:1:0}}function eachHelper(){const f=window.Handlebars;f.unregisterHelper("each"),f.registerHelper("each",function(e,t){var r,n,a="",o=t.hash.limit||void 0,i=t.hash.startAt||0,s=t.hash.sortBy||"",l=t.hash.descending||!1,u=t.hash.sortBeforeLimit||!0,c=t.inverse,p=t.fn;if(e instanceof Map&&(e=Array.from(e.values())),u="boolean"!=typeof u||u,t.data&&t.ids&&(n=(t.data.contextPath||"")+t.ids[0]+"."),f.Utils.isFunction(e)&&(e=e.call(this)),t.data&&(r=f.createFrame(t.data)),!Array.isArray(e)&&"object"!=typeof e)return c(this);var h=Array.isArray(e)?e.slice():Object.entries(e);void 0===o&&(o=h.length),u?(h.sort(sortFunction(s,l)),h=h.slice(i,o+i)):(h=h.slice(i,o+i)).sort(sortFunction(s,l));for(var d=0;d<h.length;d++){var g=Array.isArray(e)?h[d]:h[d][1];r.key=Array.isArray(e)?d:h[d][0],r.index=d,r.first=0===d,r.last=d===h.length-1,n&&(r.contextPath=n+r.key),a+=p(g,{data:r,blockParams:f.Utils.blockParams([g,r.key],[n+r.key,null])})}return 0===d?c(this):a})}function executeHelper(){window.Handlebars.registerHelper("execute",function(t,...e){if(ConfigManager.getConfig().allowExecute){e.pop();var t=String(t).trim(),r=ALLOWED_FUNCTIONS.get(t);if(r)try{return r.apply(this,e)}catch(e){return Debug.error("Error executing registered function "+t+": "+e.message),""}r=this[t];if("function"==typeof r)try{return r.apply(this,e)}catch(e){return Debug.error("Error executing context function "+t+": "+e.message),""}Debug.error("Function not found or not allowed: "+t+". Please register the function using registerExecutableFunction() before using it in a template.")}else Debug.error("execute is disabled. Cannot execute functions.");return""})}function setHelper(){window.Handlebars.registerHelper("set",function(e,t,r){return e&&t?/^[a-zA-Z_][a-zA-Z0-9_]*$/.test(e)?void(r.data.root[e]=t):(Debug.error(`setHelper: varName ${e} is not a valid variable name`),""):(Debug.error("setHelper: varName and varValue are required"),"")})}function bunnyHelper(){const t=window.Handlebars;void 0===t?console.error("Handlebars is not loaded yet!"):t.helpers.bunny||(window._fp_bunnyStates={bunny:`
    &nbsp;&nbsp;&nbsp;&nbsp;/)  /)<br>
    ପ(˶•-•˶)ଓ ♡<br>
    &nbsp;&nbsp;&nbsp;/づ  づ
  `,bunnyFlipped:`
...
for(const l of Object.keys(e))l in t||(l.startsWith("w-")||"class"===l?"class"===l&&(a=(e[l]||"").split(" ").filter(Boolean),o=(t[l]||"").split(" ").filter(Boolean),a=a.filter(e=>e.startsWith("w-")),(o=[...new Set([...o,...a])]).join(" ")!==e[l])&&(i[l]=o.join(" "),s=!0):(i[l]="",s=!0));return s?i:null}static diffEvents(e,t){var r,n,a={},o=[];let i=!1;for([r,n]of Object.entries(t))e[r]!==n&&(a[r]=n,i=!0);for(const s of Object.keys(e))s in t||(o.push(s),i=!0);return i?{...0<Object.keys(a).length&&{add:a},...0<o.length&&{remove:o}}:null}static patch(e,t){Performance.start("VirtualDOM.patch"),console.debug(`VirtualDOM: Processing ${t.length} total patches`);const r={replaces:[],moves:[],removes:[],creates:[],updates:[]};t.forEach(e=>{switch(e.type){case"REPLACE":r.replaces.push(e);break;case"MOVE":r.moves.push(e);break;case"REMOVE":r.removes.push(e);break;case"CREATE":r.creates.push(e);break;default:r.updates.push(e)}}),console.debug("VirtualDOM: Patch groups:",{replaces:r.replaces.length,moves:r.moves.length,removes:r.removes.length,creates:r.creates.length,updates:r.updates.length}),this.applyReplaces(r.replaces),this.applyRemoves(e,r.removes),this.applyCreates(e,r.creates),this.applyMoves(e,r.moves),this.applyUpdates(r.updates),Performance.end("VirtualDOM.patch")}static applyReplaces(e){e.forEach(e=>{var t;e.target&&e.vnode&&e.target.parentNode&&(t=this.createElementFromVNode(e.vnode),this.cleanupElement(e.target),e.target.parentNode.replaceChild(t,e.target),console.debug("VirtualDOM: Successfully replaced element"))})}static applyMoves(r,e){e.forEach(e=>{var t;e.target&&void 0!==e.newIndex&&(t=Array.from(r.childNodes),r.insertBefore(e.target,t[e.newIndex]||null))})}static applyUpdates(e){e.forEach(e=>{switch(e.type){case"UPDATE_ATTRS":e.target&&e.attrs&&this.updateElementAttributes(e.target,e.attrs);break;case"UPDATE_EVENTS":e.target&&e.events&&this.updateElementEvents(e.target,e.events);break;case"UPDATE_WEBFLOW_STATE":e.target&&this.updateWebflowFormVisualState(e.target)}})}static applyCreates(n,e){e.sort((e,t)=>(e.newIndex??0)-(t.newIndex??0)).forEach(e=>{var t,r;e.vnode&&(t=this.createElementFromVNode(e.vnode),void 0!==e.newIndex?(r=Array.from(n.childNodes),n.insertBefore(t,r[e.newIndex]||null)):n.appendChild(t))})}static applyRemoves(n,e){console.debug(`VirtualDOM: Attempting to remove ${e.length} elements`);var t=e.filter(e=>e.target&&e.target.parentNode),r=e.filter(e=>void 0!==e.textNodeIndex),e=e.filter(e=>!e.target&&void 0!==e.oldIndex),r=(console.debug("VirtualDOM: Remove breakdown:",{elementRemoves:t.length,textNodeRemoves:r.length,indexRemoves:e.length}),[...r.map(e=>({...e,isTextNode:!0})),...e.map(e=>({...e,isTextNode:!1}))].sort((e,t)=>(t.textNodeIndex??t.oldIndex??-1)-(e.textNodeIndex??e.oldIndex??-1)));t.forEach((e,t)=>{console.debug(`VirtualDOM: Remove element operation ${t+1}:`,{hasTarget:!!e.target,hasParent:!(!e.target||!e.target.parentNode),targetElement:e.target}),e.target&&e.target.parentNode&&(this.cleanupElement(e.target),e.target.parentNode.removeChild(e.target),console.debug("VirtualDOM: Successfully removed element:",e.target))}),r.forEach(e=>{var t=Array.from(n.childNodes),r=e.textNodeIndex??e.oldIndex,t=t[r];t?e.isTextNode&&t.nodeType===Node.TEXT_NODE?(n.removeChild(t),console.debug(`VirtualDOM: Successfully removed text node at index ${r}: "${t.textContent}"`)):e.isTextNode||t.nodeType!==Node.ELEMENT_NODE?console.warn(`VirtualDOM: Node type mismatch at index ${r}. Expected ${e.isTextNode?"text":"element"}, found:`,t):(this.cleanupElement(t),n.removeChild(t),console.debug(`VirtualDOM: Successfully removed element by index ${r}:`,t)):console.warn("VirtualDOM: Failed to find node at index "+r)})}static createElementFromVNode(e){if("string"==typeof e)return document.createTextNode(e);const t=e.isSVG?document.createElementNS(this.SVG_NAMESPACE,e.tag):document.createElement(e.tag);return this.updateElementAttributes(t,e.attrs),e.events&&this.updateElementEvents(t,{add:e.events}),e.children.forEach(e=>{e=this.createElementFromVNode(e);t.appendChild(e)}),e.isFormElement&&this.updateWebflowFormVisualState(t),e.element=t}static updateElementAttributes(r,e){const n=r.namespaceURI===this.SVG_NAMESPACE;let a=!1;Object.entries(e).forEach(([e,t])=>{""===t?r.removeAttribute(e):n?r.setAttributeNS(null,e,t):r.setAttribute(e,t),"checked"===e&&r instanceof HTMLInputElement&&(a=!0)}),a&&this.updateWebflowFormVisualState(r)}static updateElementEvents(r,e){let n=this.elementEventMap.get(r);n||(n=new Set,this.elementEventMap.set(r,n)),e.remove&&e.remove.forEach(e=>{n.has(e)&&(r.removeAttribute(e),n.delete(e))}),e.add&&Object.entries(e.add).forEach(([e,t])=>{r.setAttribute(e,t),n.add(e)})}static updateWebflowFormVisualState(e){var t,r;e instanceof HTMLInputElement&&("checkbox"!==e.type&&"radio"!==e.type||(t=e.closest("checkbox"===e.type?".w-checkbox":".w-radio"))&&(t=t.querySelector(`.w-${e.type}-input`))&&(r=e.checked,console.debug(`VirtualDOM: Syncing Webflow visual state for ${e.type} - checked: `+r),r?t.classList.add("w--redirected-checked"):t.classList.remove("w--redirected-checked")))}static cleanupElement(t){var e=this.elementEventMap.get(t);e&&(e.forEach(e=>{t.removeAttribute(e)}),this.elementEventMap.delete(t)),t.querySelectorAll("*").forEach(t=>{var e=this.elementEventMap.get(t);e&&(e.forEach(e=>{t.removeAttribute(e)}),this.elementEventMap.delete(t))})}}Object.defineProperty(VirtualDOM,"VOID_ELEMENTS",{enumerable:!0,configurable:!0,writable:!0,value:new Set(["area","base","br","col","embed","hr","img","input","link","meta","param","source","track","wbr"])}),Object.defineProperty(VirtualDOM,"SVG_NAMESPACE",{enumerable:!0,configurable:!0,writable:!0,value:"http://www.w3.org/2000/svg"}),Object.defineProperty(VirtualDOM,"SVG_ELEMENTS",{enumerable:!0,configurable:!0,writable:!0,value:new Set(["svg","circle","ellipse","line","polygon","polyline","rect","path","g","text","tspan","defs","use","symbol","marker","clipPath","mask","pattern","image","foreignObject"])}),Object.defineProperty(VirtualDOM,"elementEventMap",{enumerable:!0,configurable:!0,writable:!0,value:new WeakMap});const _readyState={isReady:!1,queue:[],plugins:new Set,hasPluginsRegistered:!1,processQueue(){for(Debug.debug(`Processing ${this.queue.length} queued operations`);0<this.queue.length;){var e=this.queue.shift();try{e()}catch(e){Debug.error("Error processing queued operation:",e)}}},enqueue(e){if(this.isReady)try{e()}catch(e){Debug.error("Error executing operation:",e)}else this.queue.push(e)},markReady(){0!==this.plugins.size||this.hasPluginsRegistered||Debug.info("No plugins registered, proceeding with initialization"),this.isReady=!0,this.processQueue()},registerPlugin(e){this.hasPluginsRegistered=!0,this.plugins.add(e),Debug.debug(`Plugin registered: ${e}, total plugins: `+this.plugins.size)},unregisterPlugin(e){var t=this.plugins.delete(e);return t?(Debug.debug(`Plugin unregistered: ${e}, remaining plugins: `+this.plugins.size),0===this.plugins.size&&(this.hasPluginsRegistered=!1,Debug.debug("All plugins unregistered, reset plugin registration state"))):Debug.warn("Attempted to unregister non-existent plugin: "+e),t},reset(e=!0){this.queue=[],this.plugins.clear(),this.hasPluginsRegistered=!1,e?Debug.info("ReadyState reset but maintains ready status for new plugins"):(this.isReady=!1,Debug.info("ReadyState completely reset, FlowPlater needs re-initialization"))},cleanup(){this.reset(!1),Debug.info("FlowPlater ReadyState cleaned up completely")}},VersionManager={parseVersion(e){return e.split(".").map(Number)},compareVersions(e,t){var r=this.parseVersion(e),n=this.parseVersion(t);for(let e=0;e<3;e++){if(r[e]>n[e])return 1;if(r[e]<n[e])return-1}return 0},satisfiesVersion(e,t){var r;return!(!e||!t)&&([r,e]=e.split("@"),!e||0<=this.compareVersions(t,e))}},PluginManager={plugins:new Map,globalMethods:new Map,instanceMethods:new Map,customTransformers:new Map,registerPlugin(e,t={}){var r,n,a;if(!(r="string"==typeof e?window[e]:e))throw new FlowPlaterError("Plugin not found: "+e);if("function"!=typeof r)throw new FlowPlaterError("Plugin must be a valid function");const o=r(t);for(const m of["name","enabled","priority","version"])if(!(m in o.config))throw new FlowPlaterError(`Plugin config must contain '${m}'`);if(!/^\d+\.\d+\.\d+$/.test(o.config.version))throw new FlowPlaterError(`Plugin version must be in format 'x.y.z' (got ${o.config.version})`);if(this.plugins.has(o.config.name))throw new FlowPlaterError(`Plugin ${o.config.name} already registered`);if(o.config.dependencies)for(const b of o.config.dependencies){var[i,s]=b.split("@"),l=this.getPlugin(i);if(!l)throw new FlowPlaterError(`Required dependency '${i}' not found for plugin `+o.config.name);if(!VersionManager.satisfiesVersion(b,l.config.version))throw new FlowPlaterError(`Plugin ${o.config.name} requires ${i} version ${s||"any"}, but found version `+l.config.version)}if(o.config.optionalDependencies)for(const v of o.config.optionalDependencies){var[u,c]=v.split("@"),p=this.getPlugin(u);p&&!VersionManager.satisfiesVersion(v,p.config.version)&&Debug.warn(`Optional dependency '${u}' version mismatch for plugin ${o.config.name}. Required: ${c||"any"}, Found: `+p.config.version)}if(o.inheritableAttributes&&Array.isArray(o.inheritableAttributes)&&o.inheritableAttributes.forEach(e=>{AttributeMatcher.addInheritableAttribute("fp-",e)}),_readyState.registerPlugin(o.config.name),o.globalMethods)for(const[y,E]of Object.entries(o.globalMethods)){if("function"!=typeof E)throw new FlowPlaterError(`Global method ${y} must be a function`);if(this.globalMethods.has(y))throw n=this.globalMethods.get(y),new FlowPlaterError(`Global method ${y} already registered by plugin `+n.plugin);this.globalMethods.set(y,{method:E,plugin:o.config.name}),"undefined"!=typeof window&&window.FlowPlater&&(window.FlowPlater[y]=(...e)=>this.executeGlobalMethod(y,...e))}if(o.instanceMethods)for(var[h,d]of Object.entries(o.instanceMethods)){if("function"!=typeof d)throw new FlowPlaterError(`Instance method ${h} must be a function`);if(this.instanceMethods.has(h))throw a=this.instanceMethods.get(h),new FlowPlaterError(`Instance method ${h} already registered by plugin `+a.plugin);this.instanceMethods.set(h,{method:d,plugin:o.config.name})}if(o.helpers&&"object"==typeof o.helpers)for(var[g,f]of Object.entries(o.helpers))if("function"!=typeof f)Debug.warn(`Plugin ${o.config.name} contains invalid helper ${g}:`,f);else try{Handlebars.registerHelper(g.toLowerCase(),f)}catch(e){Debug.error(`Plugin ${o.config.name} failed registering helper ${g}:`,e)}if(this.plugins.set(o.config.name,o),_readyState.isReady){if(o.config?.enabled&&"function"==typeof o.init&&o.init(),this.updateExistingInstances(),o.hooks?.initComplete)try{o.hooks.initComplete(window.FlowPlater,Object.values(_state.instances))}catch(e){Debug.error(`Plugin ${o.config.name} failed executing initComplete:`,e)}}else _readyState.queue.push(()=>{if(o.config?.enabled&&"function"==typeof o.init&&o.init(),this.updateExistingInstances(),o.hooks?.initComplete)try{o.hooks.initComplete(window.FlowPlater,Object.values(_state.instances))}catch(e){Debug.error(`Plugin ${o.config.name} failed executing initComplete:`,e)}});return o},updateExistingInstances(){Object.values(_state.instances).forEach(t=>{for(const[r,e]of this.instanceMethods.entries())t[r]||(t[r]=(...e)=>this.executeInstanceMethod(r,t,...e))})},getPlugin(e){return this.plugins.get(e)},getAllPlugins(){return Array.from(this.plugins.values())},getEnabledPlugins(){return this.getAllPlugins().filter(e=>e.config?.enabled)},removePlugin(e){var t,r,n,a,o,i,s=this.getPlugin(e);if(!s)return!1;s.inheritableAttributes&&Array.isArray(s.inheritableAttributes)&&s.inheritableAttributes.forEach(e=>{AttributeMatcher.removeInheritableAttribute("fp-",e)});const l=new Set;for([t,r]of this.instanceMethods.entries())r.plugin===e&&l.add(t);for([n,a]of this.globalMethods.entries())a.plugin===e&&(this.globalMethods.delete(n),"undefined"!=typeof window)&&window.FlowPlater&&delete window.FlowPlater[n];for([o,i]of this.instanceMethods.entries())i.plugin===e&&this.instanceMethods.delete(o);return Object.values(_state.instances).forEach(t=>{l.forEach(e=>{delete t[e]})}),_readyState.unregisterPlugin(e),this.plugins.delete(e)},disablePlugin(e){e=this.getPlugin(e);return!!e&&!(e.config.enabled=!1)},enablePlugin(e){e=this.getPlugin(e);return!!e&&(e.config.enabled=!0,"function"!=typeof e.init||e.state?.initialized||(e.init(),e.state.initialized=!0),!0)},pluginConfig(e){e=this.getPlugin(e);return e?e.config:null},getSortedPlugins(){return this.getEnabledPlugins().sort((e,t)=>(t.config?.priority||0)-(e.config?.priority||0))},_determineDataType(e){if(("object"!=typeof e||null===e)&&"string"==typeof e)try{JSON.parse(e)}catch(e){return"html"}return"json"},applyTransformations(a,e,o,i="json"){if(!a||"object"!=typeof a)return Debug.error("Invalid instance provided to applyTransformations"),e;Debug.debug("[Transform] Starting "+o,{instance:a.instanceName,isEvent:e instanceof Event,dataType:typeof e,isProxy:e&&"object"==typeof e&&"toJSON"in e});e=this.getSortedPlugins().reduce((e,t)=>{if(t.transformers&&"function"==typeof t.transformers[o])try{Debug.debug(`[Transform] Applying ${t.config.name}'s `+o,{dataType:typeof e,isEvent:e instanceof Event});var r=(0,t.transformers[o])?.(a,e,i);return null==r?(Debug.warn(`Plugin ${t.config.name} returned undefined/null for ${o}, using original data`),e):
